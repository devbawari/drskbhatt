'use client';

import Link from 'next/link';
import { Heart, Phone, Mail, MapPin, ChevronUp } from 'lucide-react';
import './Footer.css';

const FacebookIcon = ({ size = 18 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
);
const InstagramIcon = ({ size = 18 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
);
const YoutubeIcon = ({ size = 18 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
);

export default function Footer() {
  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  return (
    <footer className="footer">
      <div className="container">
        <div className="footer-grid">
          {/* Brand */}
          <div className="footer-brand">
            <h3>
              <img src="/icon.png" alt="Vardaan Logo" style={{ width: 24, height: 24, display: 'inline', marginRight: '8px', verticalAlign: 'middle' }} />
              Dr. SK Bhatt - Vardaan Homeopathy Clinic
            </h3>
            <p>
              Dedicated to Your Health with Compassionate, Personalized Homeopathic Care. Book Your Appointment Today!
            </p>
            <div className="footer-social">
              <a href="https://facebook.com/drskbhatt" target="_blank" rel="noopener noreferrer" className="footer-social-icon" aria-label="Facebook">
                <FacebookIcon size={18} />
              </a>
              <a href="https://www.instagram.com/dr_skbhatt?utm_source=qr" target="_blank" rel="noopener noreferrer" className="footer-social-icon" aria-label="Instagram">
                <InstagramIcon size={18} />
              </a>
              <a href="https://youtube.com/@drskbhatt?si=xyiF1CCctj26UZj1" target="_blank" rel="noopener noreferrer" className="footer-social-icon" aria-label="YouTube">
                <YoutubeIcon size={18} />
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div className="footer-col">
            <h4>Quick Links</h4>
            <div className="footer-links">
              <a href="#home" className="footer-link">Home</a>
              <a href="#about" className="footer-link">About</a>
              <a href="#services" className="footer-link">Services</a>
              <a href="#testimonials" className="footer-link">Testimonials</a>
              <a href="#contact" className="footer-link">Contact</a>
              <Link href="/booking" className="footer-link">Book Appointment</Link>
            </div>
          </div>

          {/* Services */}
          <div className="footer-col">
            <h4>Services</h4>
            <div className="footer-links">
              <a href="#services" className="footer-link">Arthritis Treatment</a>
              <a href="#services" className="footer-link">Spondylitis Treatment</a>
              <a href="#services" className="footer-link">Slip Disc</a>
              <a href="#services" className="footer-link">Sciatica</a>
              <a href="#services" className="footer-link">Kidney Stones</a>
              <a href="#services" className="footer-link">Piles</a>
            </div>
          </div>

          {/* Contact Info */}
          <div className="footer-col">
            <h4>Contact Us</h4>
            <div className="footer-contact-item">
              <Phone size={16} />
              <span>+91 8808080088</span>
            </div>
            <div className="footer-contact-item">
              <Mail size={16} />
              <span>drskbhatt@gmail.com</span>
            </div>
            <div className="footer-contact-item" style={{ alignItems: 'flex-start' }}>
              <MapPin size={16} style={{ marginTop: '4px' }} />
              <div>
                <strong style={{ display: 'block', color: 'var(--color-accent)', fontSize: '13px' }}>Morning Clinic (10:30AM-1:30PM)</strong>
                <span style={{ fontSize: '13px' }}>UGF- 8, sector -k1 smriti plaza, Ashiyana, Lucknow, UP 226012</span>
              </div>
            </div>
            <div className="footer-contact-item" style={{ alignItems: 'flex-start' }}>
              <MapPin size={16} style={{ marginTop: '4px' }} />
              <div>
                <strong style={{ display: 'block', color: 'var(--color-accent)', fontSize: '13px' }}>Evening Clinic (6:00PM-9:00PM)</strong>
                <span style={{ fontSize: '13px' }}>UGF-5, Sector-F, LDA Colony, Parag Dairy Road, Lucknow, UP 226012</span>
              </div>
            </div>
          </div>
        </div>

        <div className="footer-bottom">
          <p className="footer-copyright">
            © {new Date().getFullYear()} Dr. SK Bhatt — Vardaan Homeopathy Clinic. All rights reserved.
          </p>
          <button className="footer-back-top" onClick={scrollToTop} aria-label="Back to top" id="back-to-top">
            <ChevronUp size={20} />
          </button>
        </div>
      </div>
    </footer>
  );
}
